...
The flat-rotation curve problem  is  the divergence of two velocity parameters   from  different    observations of the  light from spiral galaxies
 \cite{Rub,Bosma,1985ApJAlbada}. 
 The  observations   are photometry and Doppler shifted spectra.  
Photometry gives the  expected Keplerian orbital  velocities,  which descend  past   the stars, Fig.~\ref{fig:NGC2403RCFM}. 
 Doppler shifted   spectra gives
flat-rotation curve velocities, which   remain ``flat''  far past the stars. 
The divergence of these two velocities is 
 primary evidence for dark matter theory.
 
 Dark matter  is   hypothesized to be    massive particles which are electromagnetically neutral, and    have a very low interaction probability with baryonic matter. However, in the absence of  a    detection event  \cite{Cebrian:2022brv},   the paradoxes of dark matter theory remain    interesting.   
 S. McGaugh  notes one such paradox:  ``Why is the luminous tail wagging the dark matter dog,  if dark matter dominates dynamics ?'' \cite{1999McGaugh,McGaugh2016RAR}. This references the    fact   that knowledge  of  the   stellar   disk   completely  determines the spherical dark matter halo\cite{2004ApJ...609..652M}.
Another  paradox is called the Universal Rotation Curve (URC)  \cite{salucci,Persic,1978Rubin,10.1111/j.1365-2966.2007.11696.x}, which is  a spectrum of   1,100  rotation curves normalized by scale-length (See Fig.~\ref{fig:URC}) .  It is a paradox because the galaxies larger than our own have rotation curves which inflect down from ``flat'', and those smaller than our Galaxy have curves which inflect up towards ``flat''. 
In dark matter theory this implies
large galaxies  require ``minimal dark-matter halos'', and small galaxies are ``dark matter dominated''.  
This is because dark matter theories are built from standard  gravity but in standard gravity mass accretion rate are directly proportional to    mass\cite{10.1093/mnras/stt2403}. 
Due to such paradoxes,  dark matter theories require fine-tuning and 4 free parameters per galaxy fitted (not including estimates of luminous mass).   Hence,   a reduction in the predictivity of the models   \cite{MCGAUGH2021220}. 
...
\end{figure}
    ...
    We will instead interpret this problem   as a problem of relative frames, specifically the role of the Milky Way on our observations. We will do so by identifying the    luminosity measured from photometry as a  Lorentz scalar and Doppler shifted spectra as part of a Lorentz 4-vector, and then using    field-frames to map between slightly curved galaxy frames  in $g_{tt}$ Schwarzschild,
    one-to-one in radii. To do so, we draw inspiration from the leading   alternative     to dark matter theory,  Modified Newtonian Dynamics (MOND) \cite{Milgrom}.   In MOND,        the   luminous mass is the only mass   but  Doppler shifted spectra is still  interpreted as   a physical   velocity.  MOND explains flat-rotation curve velocities by changing the  law of gravitational acceleration   on the enormous distance scales of galaxies  \cite{McGaugh_2014}. 
MOND  is very successful  fitting most galaxies with only the luminous mass, and one,   essentially   constant free parameter representing the acceleration scale \cite{2016Lelli}. 
While MOND removes the problem of dark matter from these systems efficiently and with fewer free parameters, it   remains a   fundamental  modification of  physics, which when extended  to the relativistic regime   gives rise to  new theories of gravity  \cite{PhysRevD.70.083509,doi:10.1142/S0217751X0703666X,Famaey2012}. The formalism in this paper can be seen as  a heuristic   extension of MOND's    idea of changing acceleration,  to the relativistic paradigm of relative galaxy curvatures. It can also be seen that the role of   MOND's  single, almost constant acceleration scale is replaced by the   role of  our Milky Way on  observations.  Previous consideration of the role of relative
     galaxy curvature  in this problem were obviated 
       by  Galilean subtraction of   gravitational red-shifts at the  large $r$  limit of the data \citep{MTW}. The rotation curve fitting model (RCFM)  presented here   is as  successful as   MOND or dark matter theories in fitting galaxy rotation curves, but does not modify classical gravity.   We demonstrate     the efficiency of this  approach by 
       fitting   the SPARC \cite{2016Lelli} sample of 175 galaxy rotation curve data previously   fitted by the MOND, dark matter models, and an evolution of MOND called  the    RAR (Radial Acceleration Relation) \cite{McGaugh2016RAR,2016Lelli,McGaugh_2014,Li_2018}. 
...
\end{equation} 
  ...
  and 
  terms in $v(r)_{rot} $ are the  total flat-rotation velocity   prediction.  All velocities are assumed to be those of test particles in circular orbits, in the plane of the stellar disk,  about the rotation axis of the galaxy at  $r=0$. 

  
  The dark matter prediction  $v(r)_{rot} $
  is fitted to the flat-rotation curve velocity  
  $v_{obs}(r)$, which    come from a flat spacetime
 Lorentz boost  of the  in terms of the received shifted spectra data  $\omega'(r)$,  
...
\end{equation} 
  ...
  as compared to the lab frequency      $\omega_o$.   Note,  the  vacuum light speed is $c$, and the frequencies in Eq.~\ref{eq:modelLumA}} are the temporal part of a Lorentz 4-vector $k_i = (k_o, k_i)$. This form of the Lorentz boost is derived from the assumption of a boost between two inertial frames. 
...
We approach the flat-rotation curve problem not as a missing mass problem, but as a gravitational frame-dependency  due to our Milky Way.  
We directly replace 
  the dark matter term $v(r)^2_{dm}/r$ in the rotation curve in Eq.\ref{eq:zonte1},  which represents  the gradient in the gravitational potential by Eq.~\ref{zoochance1},       with  Lorentz-type mappings between the extended gravitational frames of galaxies, one-to-one in radius.  Like MOND, we   assume baryonic mass is the only mass. However, different from MOND, we use the    identification of  luminosity   as a Lorentz scalar and Doppler shifted spectra as part of a 4-vector to decompose this problem at the level of the data. As a reminder, Lorentz scalars are agreed upon by both frames, under a reliable distance indicator  (Cepheid variable stars, Tip of the Red Giant Branch), and 4-vectors transform in a Lorentzian sense.  
We   will also assume for convenience in this heuristic treatment   that the contributions to  Doppler shifted spectra from  relative frame  motion ($v_{lum}$) can be  separated from those from relative curvature roughly as in Eq.~\ref{eq:zonte1} \cite{Jack,Cisn}.  
...
Gravitational potentials calculated for  a spherically symmetric  geometry   
  merge to those calculated for an exponential disk  at lengths greater than one-third of the exponential scale-length of the disk $R_e$, ie.  $r> R_e/3$\cite{Chatterjee}. This is the region  where dark matter effects becomes important \cite{1985ApJAlbada}, thus we conclude that this simplying assumption is justified for proof of concept. 
%ASK STACY:   \textcolor{purple}{AR: why divide by two?}   {\color{teal} YO:  this interpretation:  the mass in the inner $1/3$ of the stellar disk is  where  rotation curve fits   set the mass-to-light ratios.  }  
However,  in the inner region of a galaxy $r< R_e/3$,  the spherical assumption   
    overestimates the potential of a thin disk by a factor of $\approx  2$, see Fig.~\ref{fig:my_geom}, which may impact the rather high mass-to-light ratio results from RCFM fits as presented here. 
...
%  \begin{figure}
 %   \centering
  ...
  %  \caption{[MAKE:    Milky Way graphs,  Lum
  %   Xue-Sofue and McGaugh \cite{sofue2009unified}, \cite{McGaugh_2019} }
...
%{\color{blue}\emph{(ASK STACY about this interpretation)} }.  
...
There are two metrics by which rotation curve fits are judged. One, the reduced  $\chi^2$ values, and 
 ...
 %{\color{teal} Ask Stacy about this interpretation: The $\gamma_d$ for the RCFM   are consistent with PSM within the stated error of $ 20\%$ \cite{2016Lelli} after scaling the average disk $\gamma_d$ down by a factor of 2 to compensate for the spherical integration procedure. }
   ...
   \subsection{Comparing Milky Way Models} 
    ...
    The residuals of fits to the SPARC sample were used to compare different Milky Way models (see Sec.~\ref{MWselect}).  Histograms of residuals normalized by the error in velocity observations are shown in Fig.~\ref{fig:residual graphs}. 
     ...
     %{\color{blue}Ask Stacy about this interpretation: } 
...
\begin{minipage}{.5\textwidth}
  ...
  \captionof{figure}{ RCFM fit to Bulge dominated NGC 7814,   rotation curve data from SPARC\cite{2016Lelli}.}
...
\begin{minipage}{.5\textwidth}
  ...
  \captionof{figure}{  RCFM fit to Disk dominated NGC 891, rotation curve data from  SPARC\cite{2016Lelli},    }
  \label{fig:test1}
\end{minipage}
...
ratios of terms for    the Milky Way with respect to  the galaxy being observed. This may mean that the free parameter in Eq.~\ref{FreeParamFix}  involves a ratio of terms in $(L_{total}/R_{eff})_{mw} $ with respect to the
 the Milky Way.
 %{\color{blue}
 %Ask Stacy: estimates of $L_{tot}$ for the MW range from 200 billion M-sol, to 20 billion - the first number makes sense for this number (the quantity for the Milky Way  $42.94 =(L_{total}/R_{eff})_{mw} $, but if it's 20, and half-light radius is around 4 or 5, then this works}. 
...
%for completeness but do not consider them in our analysis.
 ...
 \section{  Conclusions \label{sec:conclu}  }
    ...
    At this time, there are many problems in   the concordance model of   cosmology which are attributed to dark matter \cite{2010dmp..book.....S,Tully:2014gfa,Naidu_2022}.  In
 this paper we  address only the flat-rotation curve problem, as it is  simplest. 
 In another paper,  we will address another paradox of galaxy rotation curves  called the  Tully-Fisher relation \citet{1977A&A....54..661T}. 
 The  rotation curve fitting model (RCFM) presented here  replaces dark matter with  a frame-dependent picture where excesses in Doppler shifted spectra are attributed to  relative    gravitational frames of galaxies. 
  Upcoming large surveys of the Milky Way  can falsify this model  by  a well-determined rotation curve observation  of the Milky Way beyond  
  our position at $\approx 8 kpc$ \cite{Ivezić_2019,2022ApJS..259...35A,2010ApJ...716....1B,de_Blok_2010}.  
  As can be seen in Fig.~\ref{fig:mwSofue}, 
the rotation curve data for  the Milky Way  is currently ambiguous  past  $8$kpc. 
...
%ASK STACY {\color{blue}(  MOND: distance and acceleration scale )}  {\color{blue}(  DM: variously 2-4??)} {\color{blue}( RCFM: MW and $\alpha = L/r_{eff} $)}.

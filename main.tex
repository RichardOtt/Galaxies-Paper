 ...
 \citet{McGaugh2016RAR} report that  the most likely values for the mass-to-light ratios for all morphological types of galaxies at a wavelength of  $[3.6]$ are
  ...
  In Sec.~\ref{GeomSphere}, we discuss using the spherical approximation for the exponential disk for computational purposes. We note that while our average $\gamma_{d}= 1.12 M_\odot/L_\odot$ is high compared to PSM, the spherical approximation we use
    ...
    \begin{table}[h!]
        ...
        \hline
         ...
         \hline
           ...
           \hline
        \end{tabular}
        \caption{Average mass-to-light ratio results from fits to the SPARC sample of 175 galaxies. The $\gamma_$ are in units of  solar mass per solar luminosity $M_\odot/L_\odot$. }
        \label{tab:lobes}
    \end{table}
   ...
   \subsection{Comparing Milky Way Models} 
    ...
    The residuals of fits using different Milky Way models (see Sec.~\rev{MWselect}) were compared to determine which resulted in the best fits.  Histograms of residuals normalized by the error in velocity observations are shown in Fig.~\ref{fig:residual graphs}. 
...
and has a reduced $\chi^2$ which is slightly better than the RAR fit to the same data (1.30 vs. 1.52). The resulting $\gamma_d$ for the RCFM and RAR fits to this galaxy are commensurate.   The fits are compared side-by-side in  Fig.~\ref{fig:CompareNGC2841}. 
 ...
 \begin{figure*}[ht] 
  \begin{subfigure}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=0.85\linewidth]{figures/NGC2841_rotmod_XueSofue.png} 
  ...
  \begin{subfigure}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=0.85\linewidth]{figures/NGC2841_RarResults_Li2018}
...
\cite{Gent}, as the preferred distance is 2$\sigma$ smaller than that reported by the most reliable distance indicator of Cepheid variable stars. RAR fits reproduce this distance preference, using a distance of $ D = 10.4 \pm 0.4$ Mpc  \cite{Li_2018}. With this fit,  RAR gives a reduced  $\chi^2 = 2.057$. $\gamma_d = 0.77 \pm 0.03$. The RCFM fit to  NGC 3198 at the Cepheid  distance  of $13.8$ Mpc, yields  a better reduced $\chi^2 = 1.676$, and mass-to-light ratios of  $\gamma_d = 0.878$(not scaled by 1/2 for geometric considerations).   See Fig.~\ref{fig:NGC2403}. 
...
To test a guess at a functional interpretation of the model's free-parameter, we select a
  subset of galaxies in Table~\ref{tab:Tset}.
  The  subset is selected based on
   galaxies that have highly accurate distances:   tip of the red giant branch, and Cepheids,  with errors in distance on the order of $5\% - 10\%$ \cite{2016Lelli}. {\color{blue}
   (CHECK YO: did we include from supernova)}. Of these, we   exclude galaxies  which have an inclination greater than $85^o$ as impossible to constrain a proper surface brightness profile, and those at inclinations less than $35^o$ as being impossible to accurately report line of sight Doppler shifts.  We also reject  three galaxies with significant non-circular motions and poor quality factors in the SPARC database.  
  By this process, we select a subset of 43 galaxies. 
...
at a confidence level of $83.3\%$, see Fig.~\ref{alpha2}.
  
 \begin{figure*}[h]
%\scalebox{0.5}%
\includegraphics[width=\linewidth]{alphavL_Reff.png} 
\caption{  Free-parameter correlation, $\alpha$ v.  a proxy for the gradient in the potential $L/R_e$.   }
\label{alpha2}
\end{figure*}  
 


We might guess that $\alpha$ is a ratio of the same quantity for  the Milky Way $\rho_{mw}$ with respect to the other   galaxy $\rho_{gal}$  
...
The   rotation curve fitting model (RCFM) presented here      replaces dark matter with a series of  Lorentz-type transformations between  the  galaxy frame being observed and that of the Milky Way. Interpreting the flat-rotation curve problem as due to frame dependency has the advantage of higher quality fits and a smaller free-parameter space than dark matter models. The RCFM can be seen as a natural relativistic extension of MOND, and in some particular galaxies which are difficult to fit with MOND at reported distances, the RCFM proves to be more responsive. We demonstrate our fitting success on the SPARC sample of   175  well studied galaxies\cite{2016Lelli}, previously fitted by MOND and RAR. 

We   show   that RCFM fits to this sample   produces an average reduced $\chi^2$  which is roughly half that reported for MOND/RAR fits to the same.  The RCFM produces mass-to-light ratios within an acceptable range of values   and is based purely  on estimates of the luminous mass{\color{blue}(CHECK THIS AFTER RECOMPUTE THE $\chi^2$ CORRECTLY Y RUN TABLES AGAIN,asked meagan y rich 1.30.23. Yo:  look for within 20 percent como dijo Lelli)}.  The final benefit is that the RCFM does not modify current theories of gravity nor add more particles to the standard model. 

The RCFM     does   introduce a new heuristic technique for Lorentz-type transformations between  weak gravitational frames,   and additionally does require a static choice for the Milky Way's  gravitational potential due to baryons. 
In fact, the assumption of the Milky Way means that the  RCFM    is falsifiable if dark matter  is  found to be necessary in our galaxy, ie. if at large radii past the stars the Milky Way's rotation curve is flat and requires dark matter.
  At the current date, the data of our Milky Way rotation curve is ambiguous outside of our position in the galactic disk at around 8kpc (cite Xue Sofue).     Recent and ongoing    surveys of millions of  stars in the Milky Way will differentiate these models   \cite{2022ApJS..259...35A,2010ApJ...716....1B,de_Blok_2010}.  As noted by M. Juric, ''Mapping the Milky Way is one of the key science drivers for LSST.''
  
 
 Dark matter is involved in many other places in our standard model of cosmology today. We propose   that some of those dark matter problems could be approached in a similar way as this one, though would likely require more complicated geometric assumptions. However, there exist a class of dark matter problems which are embedded in the structure of our metric spacetime assumptions for current concordance cosmology. Those problems are strictly beyond the scope of the present work, and will require modification of the construction of the metric in the opinion of the authors. In example, the James Webb Space Telescope may have already falsified dark matter driven galaxy formation \cite{2022arXiv221014915H}.

...
The divergence of these two velocities is 
 ...
 Dark matter  is   hypothesized to be    massive particles which are electromagnetically neutral, and    have a very low interaction probability with baryonic matter. However, in the absence of  a    detection event  \cite{Cebrian:2022brv},   the paradoxes of dark matter theories remain    interesting.   
 S. McGaugh  notes one such paradox,   ``Why is the luminous tail wagging the dark matter dog,  if dark matter dominates dynamics ?'' \cite{1999McGaugh,McGaugh2016RAR}. This references the   curious fact   that knowledge  of  the   stellar   disk   completely  determines the spherical dark matter halo\cite{2004ApJ...609..652M}.
...
that  small galaxies smaller  are proportionally more successful at accreting dark matter halos, which contradicts   standard  gravity \cite{10.1093/mnras/stt2403}.  % where mass accretion rate are directly proportional to  initial  mass
...
The  vacuum light speed is $c$: % and we note that frequencies  are the temporal part of a Lorentz 4-vector $k_i = (k_o, k_i)$, and 
...
We reinterpret the flat-rotation curve problem not as a missing mass problem, but as a gravitational frame-dependency  due to our Milky Way.  
We   replace 
...
\subsection{Synchronizing the gravitational potentials \label{sec:gravDets}}
 ...
 To compare two galaxies using Lorentz-type transformations, we want  to use the fact  that locally all frames are inertial, and so special relativity applies locally. To compare galaxies one-to-one in radius we note that in the tetrad formalism  `` when going from
one local inertial frame at a given point to another at the same point, the fields transform
with respect to a Lorentz transformation ''\cite{jetzer}. This implies that we can map galaxies one-to-one in radius if we can compare them inertially. 
  Wolfgang Rindler's said that      ``the center of each galaxy provides a basic local standard of nonacceleration ... so then can be treated like a local inertial frame relative to its own center.''\cite{rindler2013essential}.
 And so, we  compare galaxies using Lorentz-type transformations by first synchronizing their centers so they can be compared inertially. 
 
 
 
To synchronize galaxy centers, we look at the      Schwarzschild  gravitational redshift formula
...
which    gives the change in  photon energy  from point $P1$ to $P2$ in a \emph{single}, simply connected gravitational   manifold. 
Such a manifold is  covered with a timelike Killing field $\xi^t$ \cite{Wald}, and   
Eq.~\ref{eq:grav} is derived from the conservation properties of the Killing vector field. 
 These gravitational clock terms $g_{tt}$ are related to the  norm of the Killing field in a Schwarzschild spacetime is   $|\xi^t\xi_{t}|=\sqrt{g_{tt}}$, and terms in    $g_{tt}$
    Eq.~\ref{clocktime} are in turn parametrized by the Newtonian gravitational potential $\Phi$ as in Eq.~  \ref{eq:Newt}.



 Usually  gravitational potentials of galaxies   are summed from the   large $r$ limit, where all  gravitational potentials are assumed  to be zero, in to the small $r$ limit where the potential reaches a negative maxima. This summation convention is set  to ensure a  globally  flat embedding space,  where energy goes to zero at infinity. 
  However,   since  the local environments of galaxies show highly complex streamlines  and non-flat asymptotic spacetimes \cite{Pomarede:2020pme,Hoffman:2017ako}, we drop this convention in favor of comparing galaxy potentials from the place where all clocks read the same zero, at the event horizon. 
 Since the event horizon, on the order of rotation curve observations, is essentially the $r\approx 0$ limit, we will assume that this reversal of the integration convention will synchronize galaxy centers and create comparable frames. 
...
potentials  which  still obey Poisson's equation Eq.~\ref{whatsgood} and the central force law Eq.~\ref{zoochance1}, though energy conservation must be reclaimed on scales significantly larger than individual galaxies.
...
at an $R = .81\%$.
%The negative exponent suggests that the correlation may be with a ratio of the same quantity ($L_T/R_{eff}$) for the Milky Way, in which case the Milky Way's value is $45$ish, dimensionless and no prefactor. It's exact. 
%Ask Stacy- above.
%{\color{blue}(UPDATED From Comparison Sheet, and asked Daniel Lopez make graph again(next figure) RCFM 5.29.23)}
  
 \begin{figure} 
\includegraphics[width=\linewidth]{figures/alpha_v_L_R-eff5_29_23.png} 
\caption{ Correlation of RCFM free parameter $\alpha$ with ratio of  $L$ total luminosity   in units of $10^9$ solar  $L_\odot$,  and  $R_{eff}$ is the effective or half-light radius in kpc.
 ...
 % (Training Set) 
For a full list of galaxies see  Table~\ref{tab:Tset}.  {\color{blue}REMAKE Daniel Lopez.  Errors not real,   use the  SPARC estimates of 20 for LUM??}.  }
\label{alpha2}
\end{figure}  
 ...
 % 3,679 to 4,600 kiloparsecs. ChatGPT, halflight 2-3kpc
 %total LUM 2.3 × 1010 L
 % estimates of $L_{tot}$ for the MW range from 200 billion M-sol, to 20 billion - the first number makes sense for this number (the quantity for the Milky Way  $52= =(L_{total}/R_{eff})_{mw} $, but if it's 20, and half-light radius is around 4 or 5, then this works}. 
...
%  Tully-Fisher relation \citet{1977A&A....54..661T}. 

 
%love this from Boyer:  the failure involves the invalid use of nonrelativistic physics together with a relativistic radiation system.hHhHAAAAAHAHAHAHAH BADASS. 
  
 
 
 
%ASK STACY {\color{blue}( How many free parameters for each of  MOND: distance and acceleration scale )}  {\color{blue}( and DM: variously 2-4??)} {\color{blue}( RCFM: MW and $\alpha = L/r_{eff} $)}.

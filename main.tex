...
The rotation curve velocities remain ``flat''  far past the stars. The divergence of the velocity parameters is then known as the 
  flat-rotation curve problem, and  is
 primary evidence for dark matter theories.
In the dark matter paradigm,  both the Keplerian  and the rotation curve velocities are  proxies for mass,  and so their divergence signals a   missing or ``dark'' matter content in these systems (Fig.~\ref{fig:NGC2403RCFM}). 
Dark matter theories are built to be consistent with standard Einstein gravity, though      comprised of new dark particles  which augment the standard model. The  dark matter  particles   have a very low interaction probability with baryonic matter, and so   direct detection experiments hunt to  increase sensitivity levels \cite{Cebrian:2022brv}. Meanwhile, in the absence of a direct detection event,   the paradoxes of dark matter interpretations remain    interesting.   
...
\end{figure}
 ...
 the   stellar   disk   completely  determines the spherical dark matter halo \cite{2004ApJ...609..652M}. A   related paradox is the Universal Rotation Curve (URC), which  involves a spectrum of   1,100  rotation curves normalized by scale-length and plotted on the same axes 
 (See Fig.~\ref{fig:URC}))  \cite{salucci,Persic,1978Rubin,10.1111/j.1365-2966.2007.11696.x}. Oddly, the URC spectrum   inflects about a median point of truly flat roation curves where coincedentally the Milky Way's presumed rotation curve lies.  Further,  galaxies larger than the Milky Way have 
 rotation curves which    inflect down toward the   flat   curves, and galaxies smaller than the Milky Way inflect up toward flat.
 In the dark matter paradigm there is no explanation for  this phenomena \cite{MCGAUGH2021220}. Interpreted, instead, as a frame-dependency due the Milky Way, however, the URC spectrum makes perfect sense. 
...
The   leading   alternative     to dark matter models is   Modified Newtonian Dynamics (MOND) \cite{Milgrom},  in which     the   luminous mass is the only mass,  but  there is   a changing   scale of gravitational acceleration to explain anomalous flat-rotation velocities,  which are   interpreted as physical rotations of test particles about the galactic center \cite{McGaugh_2014}. MOND introduces a new acceleration scale to successfully fit   a large number of spiral galaxies  rotation curves with essentially a single, constant acceleration scale \cite{2016Lelli}.   While MOND removes the problem of dark matter from these systems effectively, it still  fundamentally alters   physics.  Previous attempts to extend MOND into the relativistic regime have given rise to  new theories of gravity  \cite{PhysRevD.70.083509,doi:10.1142/S0217751X0703666X,Famaey2012}. 
The approach we propose is more conservative,  as it does not modify gravitational theory or require new particles, but rather transitions  the concept of   MOND's  changing acceleration scale to the relativistic regime of    relative galaxy  curvatures. 
The rotation curve fitting model (RCFM)  presented here    preserves the fitting successes of MOND, but   improves upon   some notorious problem galaxies.   We demonstrate     the efficiency of the RCFM approach by fitting   a sample of 175 galaxy rotation curves, with luminous mass models from  very accurate photometry,  previously   fitted by the MOND   and    RAR (Radial Acceleration Relation) \cite{McGaugh2016RAR,2016Lelli,McGaugh_2014,Li_2018}. 


The RCFM approach  has  the same number of free/constrained parameters as MOND and fewer than dark matter theories{\color{blue}\emph{(ASK STACY:to make sure this is true)} }.   The RCFM's free parameters are well correlated to a observable photometric quantities (Milky Way's luminous mass distribution,   total luminosity and half light radius of the external galaxy).    Previously, the role of relative
...
\end{equation} 
  ...
  $v_{obs}$. All velocities are assumed to be those of test particles in circular orbits, in the plane of the stellar disk,  about the rotation axis of the galaxy at  $r=0$. 
  

The flat rotation curve velocity terms   $v_{obs}$  are parameters in 
a
 Lorentz boost between inertial frames, for the observed  Doppler shifted spectra data  $\omega'(r)$,  
...
\end{equation} 
  ...
  where    $\omega_o$  is the
  characteristic lab  frame frequency for some well-known atomic transition, and terms in $c$ are  the constant vacuum light speed. 
 ...
 %%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%
The Keplerian velocities due to   luminous mass $v_{lum}$, Eq.~\ref{eq:zonte1},    come from observations of total light  (photometry), associated with mass by a Population Synthesis Model (PSM), and then yielding 
velocities by  Poisson dynamics. 
...
Terms in mass-to-light ratios   for the stellar bulge and for the stellar disk are   $\gamma_b$ and $\gamma_d$ respectively. 
...
The Newtonian gravitational potential solves Poisson's equation
...
\end{equation}
 ...
 for terms in  $G$    Newton's   gravitational constant, and 
...
We propose a resolution to the dark matter    paradoxes    by  attributing excesses in Doppler shifted spectra to  a frame-dependency  due to our Milky Way, not to excesses in rotation velocities.  
The   RCFM  
replaces the gradient in the gravitational potential    $v(r)^2_{dm}/r$ usually attributed to   dark matter,  with  Lorentz-type transformations between the extended gravitational frames of galaxies.  Like MOND, we   assume baryonic mass is the only mass, but unlike   MOND we do not interpret   excesses in velocity parameter  $v_{obs}$ as physical rotation velocities, but rather as excesses in Doppler spectra shifts due to relative gravitational frames. We assume that   test particles    in  orbit around     galactic centers are moving at speeds consistent with  the Keplerian velocity estimates from  total light. 
...
Fundamental to this approach is the early identification of  luminosity   as a Lorentz scalar, and Doppler shifted spectra encapsulated in  
  $v^2_{obs}/r$ as part of a 4-vector. Lorentz scalars are agreed upon by both frames under a reliable indicator of  distance (Cepheids, Tip of the Red Giant Branch), and 4-vectors transform in a Lorentzian sense.  
We   will also assume for convenience of comparison,  that the contributions to  Doppler shifted spectra from  relative frame  motion ($v_{lum}$) can be  separated from those from relative curvature roughly as in Eq.~\ref{eq:zonte1} \cite{Jack,Cisn}. 
...
for $v_{rc}$ our model prediction to be fitted to $v_{obs}$, terms in $v_{lum}$ remain defined as in 
  Eq.~\ref{eq:zonte1}, and the   term which replaces dark matter $\alpha \kappa^2 v(r)_{1} v(r)_{2}$   is a product of a free-fitting parameter $\alpha$, a curvature ratios ($\kappa$),  and two Lorentz type transformation $v_1$ and $v_2$. 
  In what follows, all   terms  can be assumed to  be functions of radius except the model's free parameter $\alpha$,  which is single valued for each galaxy fitted, the lab-frame frequency $\omega_o$, and the speed of light $c$.  
...
\end{equation}  
 ...
 To characterize    excesses in Doppler shifted spectra
  are     part of  a Lorentz 4-vector, we have investigated the space of hyperbolic transformations  \cite{Cisneros:2013vha,Cisneros:2014fea,Cisneros2015,Cisn2016}, and found the best fit function for $v_1$ to be
...
The best fit function we have found for  $v_2$ is 
...
Terms in $e^{\tau}$ are a convolution of the curved 2-frame $e^\zeta$ and flat 2-frame $e^\eta$.  That this second transformation  is necessary is evidenced by the local constancy of the speed of light. 
...
which come from estimates of the Keplerian velocities due to luminous mass
$\beta = v_{lum}/c$, written in terms of  the expected      frequency shifts $\omega_{l}$.  
By use of Eq.~\ref{eq:flat}, we    assert  that Keplerian rotation curves are     the best estimate of flatness, since dark matter is not required to  reproduce the rotation curve of our Solar System.
  ...
  are    identified  as the  frame field relationships   on  comparison between 
...
\begin{equation}
   ...
   intended to represent the change in a photon energy due to  moving from point $P1$ to $P2$ in a single gravitational potential,  in a simply connected  manifold \cite{Wald}.   This formula is based on the conservation properties of the Killing field. 
...
To  extend this formalism   to connect 
  two distinct gravitational manifolds at the same radii, we consider  Wolfgang Rindler's statement that        ``the center of each galaxy provides a basic local standard of nonacceleration ... so then can be treated like a local inertial frame relative to its own center.''\cite{rindler2013essential}.
    We interpret this statement as indicating that to synchronize galaxies is    to synchronize their centers, hence their Killing fields.  
    
  
      
      
   
  Usually, gravitational potentials of galaxies $\Phi$ are integrated  from the large $r$ limit,  where   the potential is assumed  to be zero, into the small $r$ limit,  where the potential reaches a negative maxima as   $r\to 0$. 
 This integration order  ensures   energy goes to zero at infinity,   consistent with  a globally flat embedding space. 
 However, since  
  the external environments of galaxies at the large $r$  limit   appear to be    complicated  and extremely diverse (ie. nonflat) \cite{Pomarede:2020pme,Hoffman:2017ako},   we drop this constraint as nonphysical, and instead integrate all galaxy  potentials   from a different zero (at the small $r$ limit). 
  
We posit that to  synchronize the   respective galaxy  centers (and hence the Killing fields) is to integrate potentials instead from the place where all clocks read zero, at the event horizon $R_s$,  for $g(R_s)_{tt} \to 0$, out to the limit of the photometric data.    The norm of the time Killing field in a Schwarzschild spacetime  $\sqrt{g_{tt}}$  is  parametrized with the Newtonian gravitational potential as per
    Eq.~\ref{clocktime}. 
For our purposes, the event horizon can be approximated by the smallest observed radii. 
 ...
 Reversing the integration order    in this way produces potentials which are synchronized, and  still obey the central force law for test particles moving in circular orbits in Eq.~\ref{zoochance1} and Poisson's equation Eq.~\ref{whatsgood}.
...
Compare number of free parameters:    {\color{blue}(Number of Free  MOND (distance and acceleration scale )}  {\color{blue}(Number of Free in DM m: variously 2-4??)} {\color{blue}(Number of Free  RCFM (MW and $\alpha = L/r_{eff} $)}.
Also, in the RCFM all    parameters can be constrained by photometry  (baryonic mass of the Milky Way and $\alpha \prop L/R_{eff}$), and so this provides a unique opportunity to constrain population synthesis model interpretations of photometric measurements through the orthogonal observation of  Doppler shifted spectra. 

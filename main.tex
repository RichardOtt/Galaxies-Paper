...
The rotation curve fitting model presented here requires a static choice for the baryon distribution of the   Milky Way (MW).   Mass-modeling of the MW is an under-constrained problem, due to     observing from       within  the disk  \cite{1991ARA&A..29..409F}.
 ...
 We test the RCFM fitting function to the SPARC sample using two different, static MW choices; 
  ...
  a model from Stacy McGaugh \cite{McGaugh_2019}. Both of these models us a combination of photometry of the Milky Way combined with either MOND or dark matter models interpretations of rotation curve data to produce their luminous mass models. 
   % this paper goes out to 1000 kpc o algo, but is convolved with DM, so useless \cite{Sof11}, data file at http://www.ioa.s.u-tokyo.ac.jp/~sofue/htdocs/2012DarkHalo/Table_GrandRC.dat
...
\section{Fit Analysis and Constraining the free parameter \label{sec:analysis}}
 ...
 \citet{McGaugh2016RAR} report that  the most likely values for the mass-to-light ratios for all morphological types of galaxies at a wavelength of  $[3.6]$ are
    $\gamma_{b} = 0.7 M_\odot/L$ for bulges  
    and $\gamma_{d}= 0.50 M/L [25]$ for disks.   
    As can be seen in Table~\ref{table:M2Light} for fits to the  SPARC sample , the  
    RCFM fits give average mass-to-light ratios of
    $\gamma_b = 0.81$ for bulges 
    and $\gamma_d = 0.56$ for disks  (scaled down by factor of 2 to account for geometric assumptions mentioned in Sec.~\ref{GeomSphere}).  
 These values are physically reasonable, given that 
    the error budget on luminous mass models  estimated stated in   \citet{2016Lelli} is $20\%$.  On the same sample, the 
  MOND/RAR results give 
  $\gamma_b = 0.73$ for bulges 
  and $\gamma_d = 0.64$ for disks.
  
  In RCFM fits reported here, the    bulge and disk mass-to-light ratios are allowed to vary freely from an initial value of $1.0$ ({\color{blue} Rich - is this still true with the shotgun thing you and Meagan implemented? }) (See Eq.~\ref{eq:zonte3}).  Reported gas fractions (HI scaled for Helium abundance) are fixed,  though addition of molecular gas could increase mass fractions in the inner kiloparsec of a galaxy   \cite{2004ApJ...609..652M}.
...
The equations outlined in Section \ref{sec:dos} contain four free parameters that must be determined for each galaxy: the static Milky Way baryon model, the model free parameter $\alpha$ and the mass-to-light ratios for the stellar bulge and disk, $\gamma_b$ and $\gamma_d$ respectively. To compute these values, the fitting procedure is as follows.

From the given  Milky Way model, data is  read in as $v_{lum}(r)$ for a series of measurements in radii. The galactic potential is computed as per Eq.~\ref{zoochance1} by numerically integrating
...
({\color{blue} I'm not sure we need to do all this algebra for them....??? }To calculate $v_1$ and $v_2$,   the terms $e^\zeta$ and $e^\eta$ are needed, which we rename for convenience. These are as defined in equations (REF) and (REF)
...
%%%%%%
 ...
 \label{results:MtoL}
   ...
   comparing RCFM results to those from MOND/RAR and dark matter results. 
...
$ g_{bar}$ represents   the baryons,  and  $g_{obs}= v_{obs}/r$    the rotation curve data.  


As can be seen in Eq.~\ref{Eq:RAR},  RAR employs a functional fitting form  which is similar to  the RCFM hyperbolic functions (compare to 
Eq.~\ref{eq:hyperbolica} and Eq.~\ref{eq:hyperbolico}), but the RCFM demonstrates  a more adaptive and   robust fitting formalism at given distances. 
...
NGC 2841 is a star dominated spiral galaxy, which   historically, has been regarded as a problematic case for MOND  \citet{Gent}. MOND and RAR find a comparable  fit that from RCFM for this galaxy only  after adjusting
the Cepheid based  distance of $14.1$ Mpc,  by $1\sigma$. The RCFM fit is accomplished at the Cepheid distance. The resulting $\gamma$ for RCFM and MOND/RAR are comparable.   See Fig.~\ref{fig:CompareNGC2841}. 

 \begin{figure*}[ht] 
  \begin{subfigure}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=0.75\linewidth]{figures/NGC2841_rotmod_XueSofue.png} 
    \caption{ RCFM fit at given Cepheid distance $14.1$ Mpc, $\gamma_d =0.87 $ (before scaling by 1/2)  and $\gamma_b =1.11	 $, and reduced  $\chi^2_\nu = 1.299$. Green line is the total luminous mass components} 
  ...
  \begin{subfigure}[b]{0.5\linewidth}
    ...
    \caption{RAR fit (from Pengfeng Li 2018)   ask permissions. Fit distance parameter gives  distance $15.5\pm 0.7$ Mpc, $\gamma_d =0.81 $ and $\gamma_b =	0.93 $, and reduced $\chi^2_\nu = 1.515$. purple line is the bulge, blue is the disk, and green  is the gas.} 
    \label{fig:2841Li2018Rar} 
    \vspace{4ex}
  \end{subfigure} 
  \caption{Star dominated galaxy NGC2841, Comparison of RCFM (a) and RAR (b) fits  \cite{Li_2018}. }
...
Figure \ref{fig:CompareIC2574} shows comparison fits to this galaxy for the  RCFM, NFW dark matter and RAR. 
MOND/RAR finds a good fit to this galaxy    after adjusting
the  distance and inclination by $1-1.5 \sigma$, and their disk mass-to-light 
...
RCFM fit results for this galaxy give  $\gamma_d = 1.105$, with no adjustment to the distance reported of  $3.91$ Mpc, which is from the reliable distance method of    the tip of the red giant branch. 
 ...
 \begin{figure*}[ht] 
  \begin{subfigure}[b]{0.5\linewidth}
    ...
    \caption{  RCFM fit, 
    Fit at reported   distance $3.78 ± 0.19 $ Mpc from tip of the red giant branch, $\gamma_d = 1.105464192 $ (not scaled down by a factor of 1/2 yet), and reduced $\chi^2_\nu = 2.098  $.Lines are as in Fig.~\ref{fig:ngc2841RCFMfit}
    } 
    \label{fig:IC2574} 
    \vspace{4ex}
  \end{subfigure}%% 
  \begin{subfigure}[b]{0.5\linewidth}
    ...
    \caption{NFW dark matter fit, Figure from Navarro 2017(fig 3). ask permission. Parameter values not reported, but visually dark matter overpredicts central rotation curve.} 
    \label{fig:Nav17} 
    \vspace{4ex}
  \end{subfigure} 
    \begin{subfigure}[b]{0.5\linewidth}
    ...
    \caption{  RAR fit (from Pengfeng Li 2018)   ask permissions. Fit distance parameter gives  distance $3.77 \pm 0.19$ Mpc, $\gamma_d =0.07 $, and reduced $\chi^2_\nu = 1.44$.Lines are as in Fig.~\ref{fig:2841Li2018Rar}} 
    \label{fig:Nav17} 
    \vspace{4ex}
  \end{subfigure} 
  \caption{ 
  Gas dominated IC 2574, Comparison of RCFM (a) and NFW dark matter (b) fits \cite{2017MNRAS.471.1841N} and (c) RAR. }
...
\cite{Gent}, as the preferred distance is 2$\sigma$ smaller than that reported by the most reliable distance indicator of Cepheid variable stars. RAR fits reproduce this distance preference, using a distance of $ D = 10.4 \pm 0.4$ Mpc in \cite{Li_2018}. With these considerations,  RAR gives a reduced chisquare  of $\chi^2 = 2.057$. $\gamma_d = 0.77 \pm 0.03$ 
...
RCFM fits  NGC 3198 at the Cepheid  distance  of $13.8$ Mpc, with a resulting reduced $\chi^2 = 1.676$, and mass-to-light ratios of  $\gamma_d = 0.878$(not scaled by 1/2 yet).   See Fig.~\ref{fig:NGC2403}. 
  ...
  \end{subfigure}%%
  \caption{ Rotation Curve fits to  NGC 3198, rotation curve data (  dots with  error bars) }
  \label{fig7} 
\end{figure*}
...
We emphasize that Doppler shifted spectra, which   lead to the flat-rotation curves and the dark matter problem in rotationally supported galaxies, is part of a 4-vector and so must be considered in a Lorentzian framework instead of Galilean. By replacing the  dark matter kinematic contributions with relative-galaxy frame transformations for a sample of 175  well studied galaxies, with accurate rotation curves and high quality photometry (CITE), we show that the RCFM is twice as successful  as MOND or dark matter models within an acceptable range of values (as judged by the average reduced $\chi^2$  and M/L values) (CHECK THIS AFTER RECOMPUTE THE CHISQUARES CORRECTLY Y RUN TABLES AGAIN).
 ...
 %%% NEED TO PUT MOND IN CONCLUSIONS SOMEHOW.
...
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{figures/CamB_rotmod_XueSofue.png}
  \captionof{figure}{CamB : SPARC\cite{2016Lelli}}
  \label{fig:test1}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{figures/D564-8_rotmod_XueSofue.png}
  \captionof{figure}{D564-8 : SPARC\cite{2016Lelli}}
  \label{fig:test2}
\end{minipage}
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{figures/D631-7_rotmod_XueSofue.png}
  \captionof{figure}{D631-7  SPARC\cite{2016Lelli}}
  \label{fig:test1}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{figures/DDO154_rotmod_XueSofue.png}
...
\begin{minipage}{.5\textwidth}
  ...
  \captionof{figure}{  NGC0891 SPARC\cite{2016Lelli}}
  \label{fig:test1}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
...
%% Headers and footers
%%%%%%%%%%%%%%
\caption{Comparison of Fit Results: RAR ( \textbf{Pengfei Li} ) and RCFM  } \label{table:M2Light} \\
\hline

...
%DARK MATTER
  ...
  At the level of the data, the flat-rotation curve problem of spiral galaxies is  the divergence of two velocity parameters from  different    observations of light
 \cite{Rub,Bosma,1985ApJAlbada}. 
 The two observations of light are photometry and Doppler shifted spectra. 
 Photometry is a measure of total light in different wavelengths, which give Keplerian velocities from a
Poisson  interpretation of luminous mass associated with light.  
These Keplerian orbital  velocities   descend  past   the stars. 
The counter observation,  Doppler shifted spectra,  gives   rotation curve velocities from  a   Lorentz Doppler shift formalism,   which remain ``flat''  far past the stars. 
In this paper we   identify photometric measurements as Lorentz scalars, given a reliable distance estimate,  and Doppler-shifted spectra as a part of a Lorentz 4-vector. 
 
 
 
 The flat-rotation curve problem is
 primary evidence for dark matter theories.
In the dark matter paradigm,  both the Keplerian velocity and the rotation curve velocity are  proxies for mass and so their divergence signals a   missing or ``dark'' matter component (Fig.~\ref{fig:NGC2403RCFM}). 
Dark matter theories are built to be consistent with standard Einstein gravity, though      comprised of dark particles  which augment the standard model particles. These dark particles   have a very low interaction probability with baryonic matter  \cite{Cebrian:2022brv}, and so   direct detection experiments hunt to  increase levels of sensitivity. Meanwhile,   the paradoxes of dark matter models remain    interesting.   
...
\end{figure}
 ...
 the   stellar   disk     determines the spherical dark matter halo \cite{2004ApJ...609..652M}. A seemingly related paradox,  involves a spectrum of the rotation curves of 1,100 galaxies, normalized by scale-length,  
 the Universal Rotation Curve (URC)(See Fig.~\ref{fig:URC}))  \cite{salucci,Persic,1978Rubin,10.1111/j.1365-2966.2007.11696.x}.
 
 
 In the URC, the  spectrum of 1,100 galaxy rotation curves  inflects about a median point of the Milky Way's presumed rotation curve. 
 For some unknown reason, the Milky Way is in the middle of a class of galaxies, with rotation curves which happen to be  truly flat  at large radii. 
  Galaxies larger than the Milky Way have rotation curves which  inflect down towards the median flat curves, and those smaller than the Milky Way inflect up. 
  Since, a
  priori, there appears   no physical reason in the standard gravity paradigm why this should be the case\cite{MCGAUGH2021220}, we will instead interpret these paradoxes as evidence for a frame-dependent picture. 
  
  In the frame-dependent picture,     excesses in Doppler shifted spectra, above estimates from the luminous mass,  are due to the imprint of the Milky Way's frame   on observations of extra-galactic Doppler shifted spectra.  Previously, the role of relative
     galaxy curvature  in this problem was obviated 
       ...
       We instead treat Doppler shifted spectra like part of a four-vector, and    Lorentz transform between galaxy frames 1-to-1 in radii,      in the weak field limit of spiral galaxies.
...
The   leading   alternative  model  to dark matter is called Modified Newtonian Dynamics (MOND) \cite{Milgrom},  where the   luminous mass is the only mass  but   there is   a changing   scale of gravitational acceleration\cite{McGaugh_2014}. MOND proposes that  on the vast distances of galaxies, 
     gravity must be modified, and  has successfully fit   a large number of spiral galaxies  rotation curves,  representing diverse    sizes and morphologies \cite{2016Lelli}.
      MONDian results are competitive with dark matter, judging by number of free parameters,  goodness of fits and resulting mass-to-light ratios  \cite{McGaugh2016RAR,2022A&A...664A..40M}. 
    However, the limitation of MOND is apparent in a few galaxies at well calibrated distances (Cepheid variable stars), where MOND fits require different distances.
  ...
  Previous extensions of MOND into the relativistic regime have given rise to  new theories of gravity  \cite{PhysRevD.70.083509,doi:10.1142/S0217751X0703666X,Famaey2012}, but we
       propose     a more conservative approach. Transitioning the concept of  MOND's   changing acceleration scales to the relativistic concept of  changing relative galaxy curvatures, preserves the fitting successes of MOND, but   increases sensitivity for some notorious problem cases. 
 From this perspective, we   view the MOND's  roughly constant acceleration scale free-parameter  as   characterizing the role of the Milky Way in our observations. 
  
       
       
 We    demonstrate the success of this approach by   a heuristic replacement of dark matter in the  rotation curve formula, with     Lorentz-type relative transformations between  galaxy frames,   one-to-one in radius. 
...
\begin{minipage}{.5\textwidth}
  ...
  \label{fig:test1}
\end{minipage}
 \caption{Example  RCFM rotation curve fits (blue line), points with error bars are the rotation curve data, and the input baryonic mass models is  the green lines. {\color{teal}NOTE: remove orange lines, fit through data distracts from our fit)}  }
\end{figure*}
            %%%%%%%  %%%%%%%
  %%%%%%%   %%%%%%%  %%%%%%%
  %%%%%%%  %%%%%%%  %%%%%%%

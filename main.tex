...
The rotation curve fitting model presented here requires a static choice for the baryon distribution of the   Milky Way (MW).   Mass-modeling of the MW is an under-constrained problem, due to     observing from       within  the disk  \cite{1991ARA&A..29..409F}.
 ...
 Since we assume   the dark matter problem in this context is due to the imprint of our Milky Way on the Doppler shifted spectra we receive from external galaxies, the RCFM    is   falsifiable  if dark matter is found to be necessary to explain the rotation curve of the Milky Way at distances larger than 8 kpc.  In what follows we use only estimates of the baryon distribution in the Milky Way. 
 
 
 We test the RCFM fitting function to the SPARC sample using two different, static MW models; 
   a hybrid model combined from  \citet{Xue}, 
  \citet{Sofue}, \cite{sofue2009unified}, and 
  a model from Stacy McGaugh \cite{McGaugh_2019}. 
  ...
  Notable differences between these two baryon mass models include their derivations and   phenomenology.
  The
  Xue-Sofue MW is derived from  photometric (optical and infrared) observations   used to decouple the stellar bulge and disk components from the dark matter halo inferred by rotation curve velocities reported in  \citet{Xue} SDSS DR6 rotation curve data. 
  The mass-to-luminosity (M/L) ratios   derived from near-infrared K band (2.2 µm) observations  for the bulge, within a sphere of the scale radius  $Rb = 0.5$ kpc,
are $M/L_{(bulge in 0.5kpc)} \approx 7.1 M_\odot /L_\odot$. From $20$kpc to $60$ kpc the Xue-Sofue MW comes from Xue \cite{Xue},   created from consistent physical parameters for the bulge and disk as the Sofue MW, and  decoupled  luminous mass components done by using an NFW dark matter \cite{1996ApJ...462..563N} interpretation of rotation curve data.  
In comparison, McGaugh decouples the luminous mass from rotation curve data ( CO data from \cite{2006ApJ...641..938L}),  iteratively by 
fitting their function
\begin{equation}
    g_{obs}=\frac{g_{bar}}{1-e^{-(\sqrt{g_{bar}/g_\dagger})}}
\end{equation}
to the data, where terms in $g_i$ represent the gradient of the potential as in Eq.~\ref{zoochance1}, for 
$ g_{bar}$ representative of the baryons,   $g_{obs}= v_{obs}/r$ from the flat-rotation curve data $v_{obs}$,  and $v_\dagger = 3700 km^2/ s^2 kpc$ is the MONDian-type acceleration scale. In this way, these models represent the two different current characterizations of the flat-rotation curve problem. 
...
Phenomenologically, the most important difference is that the 
  ...
  6.55 Kpc. Interestingly, the peak velocity from the disk in the Xue-Sofue MW occurs at roughly 6kpc. 
  Since the model presented here relies intrinsically on the assumed slope of the gradient in the Milky Way's potential, the difference between these two assumed Milky Way baryon models is critical. We will address goodness of fit results and the distribution of the residuals in a subsequent section. 
...
{\color{blue}\emph{COMMENT MEAGAN: We'll want a better title, and this is basically a first draft.  Also, this probably isn't the right order to do things in. Maybe we should first address which galaxies we analyze, then the actual fitting, then interpreting $\alpha?$} Comment Sofia: Im' not convinced we need to analyze individual galaxies, but it does happen in a lot of paper where they highlight individual galaxies...so let's see how it goes.}
